...
def parse_docx(path: str, meta: BookMeta) -> Book:
    ...
    markers = [(i, _marker_level(p)) for i, p in enumerate(paras)]
    markers = [(i, lv) for i, lv in markers if lv is not None]
    if markers:
        present = {lv for _, lv in markers}
        chap_level = 0 if 0 in present else 1
        head_idx = [i for i, lv in markers if lv == chap_level]
    else:
